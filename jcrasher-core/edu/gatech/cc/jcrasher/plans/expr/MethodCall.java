 ...
 */
public class MethodCall<T> implements FunctionCall<T> {
  ...
  protected void initBase(final Method pMeth, final Expression[] pConstrParams){
    notNull(pMeth);
    notNull(pConstrParams);

    returnType = (Class<T>) pMeth.getReturnType();
    meth = pMeth;
  ...
  public MethodCall(
      final Method pMeth, 
      final Expression<?>[] pConstrParams)
  {
  ...
  public MethodCall(
      final Method pMeth,
      final Expression<?>[] pConstrParams,
      final Expression<?> pReceiverPlan)
  {
    initBase(notNull(pMeth), notNull(pConstrParams));
  ...
  public String toString(final Class<?> testee) {
    ...
    final String className = 
        CodeGenFct.getName(meth.getDeclaringClass(), testee);
    ...
    if (Modifier.isStatic(meth.getModifiers())) {
      ...
      if (receiverPlan instanceof ConstructorCall) {
        res.append("(" + receiverPlan.toString(testee) + ")." + meth.getName()
            + "(");
      }
      else { // A.b().conMeth(
        res.append(receiverPlan.toString(testee) + "." + meth.getName() + "(");
    ...
    for (int i = 0; i < paramPlans.length; i++) {
      ...
      res.append(paramPlans[i].toString(testee)); // value
    }
  ...
  public String toString() {
    return toString(meth.getDeclaringClass());
  }
}
